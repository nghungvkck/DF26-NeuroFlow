...
DEFAULT_MODEL_DIR = os.path.join(PROJECT_ROOT, "forecasting", "models")
DEFAULT_RESULTS_DIR = os.path.join(PROJECT_ROOT, "forecasting", "artifacts", "predictions")
DEFAULT_METRICS_DIR = os.path.join(PROJECT_ROOT, "forecasting", "artifacts", "metrics")
...
XGBOOST_PREDICTIONS_DIR = DEFAULT_RESULTS_DIR
    ...
    for path in candidates:
        ...
        try:
            ...
            if "actual" in pred_df.columns and "predicted" in pred_df.columns:
                ...
                actual = pred_df["y_true"].astype(float).values
                predicted = pred_df["y_pred"].astype(float).values
            elif "y" in pred_df.columns and "yhat" in pred_df.columns:
                actual = pred_df["y"].astype(float).values
                predicted = pred_df["yhat"].astype(float).values
            else:
                continue
