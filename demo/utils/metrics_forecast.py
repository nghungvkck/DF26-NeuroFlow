...
def _build_features_for_metrics(df: pd.DataFrame) -> pd.DataFrame:
    ...
    LightGBM model expects these 13 features:
    - is_burst, burst_ratio, is_event (event detection)
    ...
    # is_event: Flag for known events (default to 0 for real-time data)
    df['is_event'] = 0
...
def forecast_metrics_lightgbm(
    ...
    if model_dir is None:
        model_dir = os.path.join(os.path.dirname(__file__), "..", "..", "forecasting", "artifacts", "models")
    ...
    try:
        ...
        # Load LightGBM model
        model_candidates = [
            os.path.join(model_dir, "lightgbm_5m.txt"),
            os.path.join(model_dir, "lightgbm_5m_model.txt"),
            os.path.join(model_dir, "lightgbm_5m_model.pkl"),
        ]
        ...
        if model_path is None:
            ...
            return False, f"LightGBM model not found. Model dir: {model_dir}. Available: {available_files}", None
        ...
        try:
            ...
            try:
                ...
                return False, f"Failed to load model from {model_path}: txt error={str(e)}, pkl error={str(pickle_err)}", None
