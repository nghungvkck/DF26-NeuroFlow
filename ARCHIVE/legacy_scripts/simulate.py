...
def run_strategy_on_scenario(
    ...
    # FIXED: Use 2.5σ instead of 3.0σ for better sensitivity
    # 2.5σ = 98.8% confidence, catches more anomalies
    anomaly_detector = AnomalyDetector(
        window_size=50,
        zscore_threshold=2.5,  # More sensitive than standard 3.0
        iqr_multiplier=1.5,
        rate_threshold=0.5
    )
    ...
    for t in range(len(load_series)):
        ...
        # FIXED: Better validation of forecast results
        try:
            ...
            if (forecast_result and hasattr(forecast_result, 'yhat') and 
                ...
                # Get forecast value and validate it's a number
                try:
                    ...
                    if actual_forecast > 0:
                        ...
                        # Negative forecast doesn't make sense, use conservative fallback
                        actual_forecast = actual_requests * 1.1  # +10% margin
                except (ValueError, TypeError):
                    # forecast_result.yhat[0] is not a valid number
                    actual_forecast = actual_requests * 1.1
            else:
                # Invalid forecast result structure
                actual_forecast = actual_requests * 1.1
                
        except Exception as e:
            print(f"  [Warning: Forecast failed at t={t}: {e}]")
            # Conservative fallback: assume 10% increase
            actual_forecast = actual_requests * 1.1
