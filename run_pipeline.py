...
Usage:
    python run_pipeline.py [--phase-a-only | --phase-b-only | --phase-c-only]
...
def main():
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "--phase-a-only":
            ...
            run_phase_a = False
            run_phase_c = False
        elif sys.argv[1] == "--phase-c-only":
            run_phase_a = False
            run_phase_b = False
        else:
            print(f"Unknown argument: {sys.argv[1]}")
            print("Usage: python run_pipeline.py [--phase-a-only | --phase-b-only | --phase-c-only]")
            sys.exit(1)
    ...
    if run_phase_a:
        try:
            ...
            print(f"\n❌ Phase A failed: {e}")
            if not (run_phase_b or run_phase_c):
                sys.exit(1)
    ...
    if run_phase_b:
        try:
            ...
            print(f"\n❌ Phase B failed: {e}")
            if not (run_phase_a or run_phase_c):
                sys.exit(1)
    ...
    if run_phase_c:
        try:
            ...
            import traceback
            traceback.print_exc()
            if not (run_phase_a or run_phase_b):
                sys.exit(1)
